...
type IconName =
  ...
  | "chevron" | "edit" | "trash" | "more" | "moon" | "grid" | "shield";
...
const cannedDrafts: Record<string, string> = {
  nanzhi: "翻到一本旧书，夹页里落下一片颜色很浅的花瓣。它替很久以前的某个人，把春天保留到了今天。",
  yuke: "凌晨的便利店很亮。推门时风铃响了一声，像有人替夜色落了款。",
  shisui: "阳台的新芽终于伸直了腰。决定奖励它一点晨光，也奖励自己一块草莓蛋糕。",
};

const replySeeds: Record<string, (moment: Moment) => string> = {
  nanzhi: moment => `读完以后，忽然也想把这一刻记下来。${moment.author}，愿你把今天剩下的一点温柔也留给自己。`,
  yuke: moment => `看见了。${moment.author}，这句话适合留到夜深以后再读一遍。`,
...
export default function Home() {
  ...
  const updateAvatar = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const source = await resizeAvatar(file);
      const next = { ...avatarSources, [avatarTarget]: source };
      setAvatarSources(next);
      try { window.localStorage.setItem("yimeng-local-avatars", JSON.stringify(next)); } catch { setToast("头像已预览，但当前浏览器无法继续保存"); event.target.value = ""; return; }
      setToast(`${avatarTarget}的头像已更新 · 当前设备预览`);
    } catch { setToast("这张图片没有读取成功，请换一张试试"); }
    event.target.value = "";
  };
  ...
  const runReplyBatch = () => {
    ...
    window.setTimeout(() => {
      const next = responders.map(character => ({
        id: `reply-${target.id}-${character.id}-${Date.now()}`,
        momentId: target.id,
        characterId: character.id,
        author: character.name,
        initial: character.initial,
        color: character.color,
        text: replySeeds[character.id]?.(target) ?? `我看到啦，${target.author}。这件事值得好好记住。`,
      }));
      setReplyDrafts(next);
  ...
  const runBatch = () => {
    ...
    window.setTimeout(() => {
      const next = characters.filter(character => selectedCharacters.includes(character.id)).map(character => ({ id: `draft-${character.id}-${Date.now()}`, characterId: character.id, author: character.name, initial: character.initial, color: character.color, text: cannedDrafts[character.id] ?? `${character.name}写下了一段只属于此刻的心情。` }));
  ...
  const testApi = () => {
    setApiTest("testing"); window.setTimeout(() => { setApiTest("ok"); setToast("原型中的连接流程正常"); }, 1000);
  };
  ...
  return <main className="yimeng-app app-screen" data-style={style} data-palette={palette} data-avatar-shape={avatarShape}>
      ...
      <section className="feed-column">
        ...
        <div className="feed-list">{moments.length === 0 && <div className="empty-feed panel"><span><Icon name="spark" size={25}/></span><h3>这里还很安静</h3><p>发布第一条动态后，它会同步到你们两个人的设备。</p></div>}{moments.map(moment => <article className={`moment-card panel ${moment.image ? "has-image" : "text-post"}`} key={moment.id}>
          <header>
            ...
            <button className="bare-button" aria-label="更多"><Icon name="more" size={21}/></button>
          </header>
          {moment.image && <div className={`moment-art art-${moment.image}`} role="img" tabIndex={0} onDoubleClick={() => toggleLike(moment.id)} aria-label={`${moment.image === "rain" ? "雨夜窗景" : moment.image === "night" ? "夜晚路灯" : "手帐与奶茶"}，双击喜欢`}><span/><i/><b/><em/></div>}
          <p className="moment-text"><span>{moment.text}</span></p>
          <div className="moment-actions"><button type="button" aria-label="喜欢" className={moment.likedBy.includes(currentUser.name) ? "liked" : ""} onClick={() => toggleLike(moment.id)}><Icon name="heart" size={24}/></button><button type="button" aria-label="回应" onClick={() => setCommentBox(current => current === moment.id ? null : moment.id)}><Icon name="comment" size={23}/></button><button type="button" aria-label="分享"><Icon name="send" size={23}/></button></div>
    ...
    <nav className="mobile-nav panel-soft" aria-label="主导航">
      ...
      <button type="button" className="mobile-create" onClick={() => setComposerOpen(true)} aria-label="发布朋友圈"><Icon name="plus" size={24}/><span>发布</span></button>
      ...
      <button type="button" onClick={() => openProfile(currentUser.name)} aria-label="我的主页"><Avatar initial={currentUser.initial} color={currentUser.color} size="sm" src={avatarSources[currentUser.name]}/><span>我的</span></button>
    </nav>
    ...
    {profileInfo && <div className="profile-layer" role="dialog" aria-modal="true" aria-label={`${profileInfo.name}的个人主页`}>
      <section className="profile-page">
        <header className="profile-nav">
          ...
          <button type="button" className="bare-button" aria-label="主页选项"><Icon name="more" size={22}/></button>
        </header>
        <div className="profile-hero">
          <div className="profile-avatar-wrap">
            <span className="profile-avatar-ring"><Avatar initial={profileInfo.initial} color={profileInfo.color} size="xl" online src={avatarSources[profileInfo.name]}/></span>
        ...
        {profileMoments.length === 0 ? <div className="profile-empty"><span><Icon name="image" size={28}/></span><h3>还没有发布动态</h3><p>第一条动态会出现在这里。</p></div> : profileTab === "grid" ? <div className="profile-grid">{profileMoments.map(moment => <article className={`profile-tile ${moment.image ? "has-art" : "text-only"}`} key={moment.id}>{moment.image ? <div className={`moment-art art-${moment.image}`} role="img" aria-label={`${moment.author}的动态图片`}><span/><i/><b/><em/></div> : <p>{moment.text}</p>}<div className="tile-overlay"><span><Icon name="heart" size={15}/>{moment.likedBy.length}</span><span><Icon name="comment" size={15}/>{moment.comments.length}</span></div></article>)}</div> : <div className="profile-timeline">{profileMoments.map(moment => <article key={moment.id}><header><span>{moment.time}</span><b>{moment.comments.length} 条回应</b></header><p>{moment.text}</p><footer><span><Icon name="heart" size={14}/>{moment.likedBy.length}</span><span><Icon name="comment" size={14}/>{moment.comments.length}</span></footer></article>)}</div>}
        {isOwnProfile && <p className="avatar-local-note">头像图片暂存在这台设备；显示形状只改变你看到的界面。</p>}
      </section>
    </div>}
    ...
    <input ref={importRef} className="visually-hidden" type="file" accept=".json,.png,application/json,image/png" onChange={importCharacter}/><input ref={avatarInputRef} className="visually-hidden" type="file" accept="image/jpeg,image/png,image/webp" onChange={updateAvatar}/>{toast && <div className="toast panel"><span><Icon name="check" size={16}/></span><p>{toast}</p></div>}
  </main>;
}
...
function GeneratePanel({ characters, selected, toggleSelected, batchState, runBatch, drafts, setDrafts, publishDraft, publishAll, avatarSources }: { characters: Character[]; selected: string[]; toggleSelected: (id: string) => void; batchState: string; runBatch: () => void; drafts: Draft[]; setDrafts: React.Dispatch<React.SetStateAction<Draft[]>>; publishDraft: (id: string) => void; publishAll: () => void; avatarSources: Record<string, string> }) {
  return <div className="admin-page"><AdminHeading eyebrow="AI MOMENT STUDIO" title="批量生成朋友圈" description="一次请求生成多位角色，各自拆分成可编辑草稿，确认后才会出现在朋友圈。" actions={<span className="draft-count"><Icon name="edit" size={15}/>{drafts.length} 份待确认</span>}/><div className="generate-grid">
    <section className="generation-setup panel"><div className="block-title"><span>01</span><div><strong>选择角色</strong><small>本次谁来写朋友圈</small></div><button type="button" onClick={() => selected.length === characters.length ? characters.forEach(character => selected.includes(character.id) && toggleSelected(character.id)) : characters.filter(character => !selected.includes(character.id)).forEach(character => toggleSelected(character.id))}>{selected.length === characters.length ? "清空" : "全选"}</button></div>
      ...
      <div className="block-title"><span>02</span><div><strong>生成要求</strong><small>会与每个角色人设和世界书合并</small></div></div><textarea className="prompt-box" defaultValue="根据角色此刻的状态，自然地写一条朋友圈。可以分享日常、心情或一个小发现；保持角色口吻，不要解释自己是 AI。"/>
...
function RepliesPanel({ moments, characters, selectedMomentId, selectMoment, selectedCharacters, toggleCharacter, batchState, runBatch, drafts, setDrafts, publishDraft, publishAll, avatarSources }: {
  moments: Moment[];
  ...
  return <div className="admin-page reply-page">
    ...
    <div className="reply-workspace">
      <section className="generation-setup reply-setup panel">
        <div className="block-title"><span>01</span><div><strong>选择朋友圈</strong><small>真人或角色发布的动态都可以</small></div></div>
        <div className="reply-target-list">{moments.map(moment => <button type="button" key={moment.id} className={`reply-target ${selectedMomentId === moment.id ? "selected" : ""}`} onClick={() => selectMoment(moment.id)}>
        ...
        <textarea className="prompt-box reply-prompt" defaultValue="读完这条朋友圈后，以角色自己的口吻自然回应。可以接住对方的情绪或分享联想到的小事；不要重复原文，不要解释自己是 AI。"/>
...
function WorldbookPanel({ entries, setEntries, openCreate }: { entries: WorldEntry[]; setEntries: React.Dispatch<React.SetStateAction<WorldEntry[]>>; openCreate: () => void }) {
  const [scope, setScope] = useState("all"); const filtered = scope === "all" ? entries : entries.filter(entry => scope === "global" ? entry.scope === "全局" : entry.scope !== "全局");
  return <div className="admin-page"><AdminHeading eyebrow="LORE & MEMORY" title="世界书" description="把设定、共同记忆和触发规则装进生成上下文。可全局生效，也可只绑定某个角色。" actions={<button className="primary-button" onClick={openCreate}><Icon name="plus" size={16}/><span>添加条目</span></button>}/><div className="world-toolbar panel-soft"><Segmented value={scope} onChange={setScope} options={[{ value: "all", label: "全部条目" }, { value: "global", label: "全局" }, { value: "character", label: "角色专属" }]} small/><div className="world-budget"><span>扫描深度</span><strong>4</strong><i/><span>预算</span><strong>1,024 tokens</strong></div></div><div className="world-layout">
    <aside className="book-list panel"><div className="book-cover"><Icon name="book" size={21}/><div><strong>一梦间 · 共用世界书</strong><span>{entries.filter(entry => entry.scope === "全局").length} 个全局条目</span></div></div><button className="active"><span>共用世界书</span><b>{entries.filter(entry => entry.scope === "全局").length}</b></button><p>角色专属</p>{["南枝", "雨客", "时穗"].map(name => <button key={name}><span>{name}</span><b>{entries.filter(entry => entry.scope === name).length}</b></button>)}</aside>
    <section className="entry-list">{filtered.map(entry => <article className={`lore-entry panel ${entry.enabled ? "" : "disabled"}`} key={entry.id}><header><div className="entry-index">{String(entries.indexOf(entry) + 1).padStart(2, "0")}</div><div><strong>{entry.title}</strong><span><b className={entry.scope === "全局" ? "global" : "character"}>{entry.scope}</b><i>{entry.trigger}</i></span></div><Switch checked={entry.enabled} onChange={() => setEntries(current => current.map(item => item.id === entry.id ? { ...item, enabled: !item.enabled } : item))} label="切换世界书条目"/><button className="bare-button"><Icon name="more" size={18}/></button></header><p>{entry.content}</p><footer>{entry.trigger === "关键词" && <span><strong>关键词</strong>{entry.keys}</span>}<span><strong>插入位置</strong>{entry.position}</span><span><strong>优先级</strong>100</span></footer></article>)}</section>
  </div></div>;
}

function ApiPanel({ apiBase, setApiBase, apiKey, setApiKey, model, setModel, state, test }: { apiBase: string; setApiBase: (value: string) => void; apiKey: string; setApiKey: (value: string) => void; model: string; setModel: (value: string) => void; state: "idle" | "testing" | "ok"; test: () => void }) {
  return <div className="admin-page api-page"><AdminHeading eyebrow="MODEL CONNECTION" title="OpenAI 兼容接口" description="接入支持 OpenAI Chat Completions 格式的模型服务，用于生成回复与角色朋友圈。" actions={<span className={`connection-badge ${state === "ok" ? "ok" : ""}`}><i/>{state === "ok" ? "原型流程已通过" : "尚未连接"}</span>}/><div className="api-layout">
    <section className="api-form panel"><div className="api-form-head"><span><Icon name="plug" size={20}/></span><div><strong>默认模型连接</strong><small>全站 AI 生成任务共用</small></div></div><label><span>Base URL</span><small>包含 /v1；系统会调用 /chat/completions</small><input value={apiBase} onChange={event => setApiBase(event.target.value)} placeholder="https://api.example.com/v1"/></label><label><span>API Key</span><small>正式版只会保存在服务端，不会发到访客浏览器</small><div className="key-field"><input type="password" value={apiKey} onChange={event => setApiKey(event.target.value)} placeholder="sk-••••••••••••••••"/><Icon name="lock" size={16}/></div></label><label><span>模型名称</span><small>填写服务商提供的精确 model id</small><input value={model} onChange={event => setModel(event.target.value)} placeholder="model-name"/></label><div className="api-advanced"><div><span>高级参数</span><small>用于朋友圈与评论生成</small></div><label><span>Temperature</span><input type="number" defaultValue="0.85" min="0" max="2" step="0.05"/></label><label><span>最大输出</span><input type="number" defaultValue="1800" min="100"/></label></div><footer><p><Icon name="shield" size={15}/>联机阶段将通过 Edge Function 代理请求</p><button className="primary-button" onClick={test} disabled={state === "testing" || !apiBase || !model}>{state === "testing" ? <><span className="loader"/>检查配置…</> : <><Icon name="plug" size={16}/><span>测试连接流程</span></>}</button></footer></section>
    <aside className="api-aside"><section className="panel request-map"><p className="eyebrow">ONE REQUEST · MANY ROLES</p><h3>批量生成如何工作</h3><ol><li><span>1</span><p><strong>装配角色上下文</strong><small>角色卡与已触发世界书</small></p></li><li><span>2</span><p><strong>一次发送请求</strong><small>要求返回严格 JSON 数组</small></p></li><li><span>3</span><p><strong>按角色安全分拆</strong><small>每份结果变成独立草稿</small></p></li><li><span>4</span><p><strong>由管理员确认</strong><small>编辑、删掉或发布</small></p></li></ol></section><section className="panel prototype-warning"><Icon name="spark" size={19}/><p><strong>当前是界面原型</strong><span>按钮只演示连接流程，不会真的发送或保存 API Key。后端接通后，这里无需重新设计。</span></p></section></aside>
  </div></div>;
}
